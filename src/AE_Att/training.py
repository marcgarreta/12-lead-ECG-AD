...
from model_AEAtt import AEWithAttention

DATA_DIR    = os.getenv("ECG_DATA_DIR", "/fhome/mgarreta/ENTREGA/preprocessed_mimic")
RECON_DIR   = os.getenv("ECG_RECON_DIR", "/fhome/mgarreta/ENTREGA/MA_VAE_MIMIC/recons/reconstructions_AEAtt_small")
CHECK_DIR   = os.getenv("ECG_CHECK_DIR", "/fhome/mgarreta/ENTREGA/MA_VAE_MIMIC/check/checkpoints_AEAtt_small")
...
def train(model, train_loader, val_loader, device, epochs, lr, patience,
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, x in enumerate(tqdm(train_loader, desc=f"Train Epoch {epoch+1}/{epochs}"), 1):
            ...
            optimizer.zero_grad()
            recon, _attn = model(x)          # recon: (B, W, 12)
            loss = F.mse_loss(recon, x)      # use mean squared error
            recon_term = loss
            loss.backward()
            optimizer.step()
        ...
        if (epoch + 1) % 1 == 0 and base_ds is not None and sample_length is not None and window_size is not None and stride is not None and recon_dir is not None:
            ...
            for start in range(0, sample_length - window_size + 1, stride):
                ...
                with torch.no_grad():
                    recon_win, attn_weights = model(w_t)
                recon_win = recon_win.squeeze(0).permute(1,0).cpu().numpy()  # (12,W)
                # average heads & queries to get per-lead attention: mean over heads and seq
                lead_attn = attn_weights.mean(dim=1).squeeze(0).cpu().numpy().T  # (12,W)
                windows.append(recon_win)
                attn_acc.append(lead_attn)
            # stitch back
            ...
            for start in range(0, sample_length - window_size + 1, stride):
                ...
                attn_full[:, start:end] += attn_acc[idx]
                counts[:, start:end] += 1
                idx += 1
            recon_full /= counts
            attn_full /= counts
        ...
        # Plot recon loss over epochs
        epochs_list = list(range(1, len(recon_history)+1))
        plt.figure()
        plt.plot(epochs_list, recon_history, label='Recon MSE')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
...
if __name__ == "__main__":
    ...
    model = AEWithAttention(seq_len=window_size, n_leads=12, latent_dim=64)
