...
from model_graph import MA_VAE

DATA_DIR    = os.getenv("ECG_DATA_DIR", "/fhome/mgarreta/ENTREGA/preprocessed_mimic")
RECON_DIR   = os.getenv("ECG_RECON_DIR", "/fhome/mgarreta/ENTREGA/MA_VAE_MIMIC/recons/reconstructions_graph_small")
CHECK_DIR   = os.getenv("ECG_CHECK_DIR", "/fhome/mgarreta/ENTREGA/MA_VAE_MIMIC/check/checkpoints_graph_small")
...
def train(model, train_loader, val_loader, device, epochs, lr, patience, scheduler,
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, x in enumerate(tqdm(train_loader, desc=f"Train Epoch {epoch+1}/{epochs}"), 1):
            ...
            optimizer.zero_grad()
            x_mean, x_logvar, x_recon, z_mean, z_logvar, sp_w, t_attn, focus = model(x)
        ...
        if (epoch + 1) % 1 == 0 and base_ds is not None and sample_length is not None and window_size is not None and stride is not None and recon_dir is not None:
            ...
            for start in range(0, sample_length - window_size + 1, stride):
                ...
                with torch.no_grad():
                    x_mean, x_logvar, x_recon, z_mean, z_logvar, sp_w, t_attn, focus = model(w_t)
                recon_win = x_mean.squeeze(0).permute(1,0).cpu().numpy()  # (12,W)
                # spatial attention: average over heads → (n_leads, n_leads)
                sp_map = sp_w.squeeze(0).mean(dim=0).cpu().numpy()  # (n_leads, n_leads)
                # Global temporal attention: t_attn shape (1, heads, W, W)
                ta = t_attn.squeeze(0)                      # (heads, W, W)
                temp = ta.mean(dim=0)                       # (W, W) average over heads
                focus_vec = ta.mean(dim=1).cpu().numpy()                
                windows.append(recon_win)
                attn_acc.append(focus_vec)                  # list of 1D arrays
            # stitch back
            recon_full = np.zeros_like(full_sig)
            # Prepare single-channel attention map, then broadcast across leads
            attn_full = np.zeros(full_sig.shape[1])  # (L,)
            counts = np.zeros(full_sig.shape[1])
            idx = 0
            for start in range(0, sample_length - window_size + 1, stride):
                end = start + window_size
                recon_full[:, start:end] += windows[idx]
                attn_full[start:end] += attn_acc[idx]     # focus_vec (W,)
                counts[start:end] += 1
                idx += 1
            recon_full /= counts
            attn_full /= counts
            # Broadcast global focus to all leads
            attn_full = np.tile(attn_full[np.newaxis, :], (full_sig.shape[0], 1))  # (n_leads, L)
            ...
            sp_path = os.path.join(recon_dir, f"spatial_epoch{epoch+1:03d}.png")
            plt.figure(figsize=(6,6))
            plt.imshow(sp_map, cmap='viridis', aspect='auto')
            plt.title(f"Epoch {epoch+1}: Spatial Attention (avg heads)")
            plt.xlabel("Key Lead")
            plt.ylabel("Query Lead")
